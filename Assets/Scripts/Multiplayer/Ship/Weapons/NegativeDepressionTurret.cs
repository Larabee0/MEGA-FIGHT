...
public class NegativeDepressionTurret : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        TargetTraverse = Random.Range(-LeftLimit, RightLimit);
        TargetElevation = Random.Range(-MaxDepression, MaxElevation);
    }
    ...
    private float TargetTraverse = 0f;
    ...
    private void RandomIdle()
    {
        ...
        if (IsAimed)
        {
            StartCoroutine(GenerateNewTarget());
        }
        else
        {
        ...
        if(TargetElevation == elevation && TargetTraverse == limitedTraverseAngle)
        {
            isAimed = true;
        }
        else
    ...
    private IEnumerator GenerateNewTarget()
    {
        rest = true;
    ...
    private void RotateBarrelsToFaceTarget(Vector3 targetPosition)
    {
        ...
        float AcutalElevation = targetElevation = Mathf.Clamp(targetElevation, -MaxDepression, MaxElevation);
        Debug.LogFormat("Target Elevation: {0} Actual Elevation: {1}", targetElevation, AcutalElevation);
        elevation = Mathf.MoveTowards(elevation, targetElevation, ElevationSpeed * Time.deltaTime);
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        if (turretBase != null)
        {
            ...
            if (barrels != null)
            {
                ...
                if (MaxDepression < 0)
                {
                    UnityEditor.Handles.DrawSolidArc(barrels.position, barrels.right, turretBase.forward, -(MaxElevation- MaxDepression), kArcSize);
